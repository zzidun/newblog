...
4. [知乎问题:hp和联想的笔记本电脑加入白名单限制意义何在](#hp和联想的笔记本电脑加入白名单限制意义何在)
...
转载自[百家号](https://baijiahao.baidu.com/s?id=1734985489377814507&wfr=spider&for=pc), 作者:微言码道
...
> 使用源码编译软件，这是最原始的，当然也是最有难度的，对人员能力有较高的要求。
...
在Linux的通用包中，最流行的属于AppImage,Snap以及Flatpak。这些管理方式与上述的几种方式的最大差别在于：
...
```shell
sudo apt install chromium #使用APT来安装开源的chromium
```
...
> 在 Canonical 决定用 Snap 替换部分 APT, 并让 Ubuntu Store 在用户不知情或未经同意的情况下自行安装后, Linux Mint 20 系统禁止了使用apt安装 Snap Store。
...
# hp和联想的笔记本电脑加入白名单限制意义何在

来自知乎用户[老虎会游泳的回答](https://www.zhihu.com/question/42977187/answer/124374349)

“理想和完整”？当我一脸愤怒的刷着白名单的时候，我体验到的可不是这些。我只是感觉联想掉钱眼里罢了。把好端端的`PCI-E`标准弄成了什么样子？凭什么拒绝我这张完全符合`PCI-E`标准的网卡？凭什么？这是重视用户体验的态度吗？把消费者放在眼里吗？

这是我的电脑，我花钱购买的设备，我应当拥有控制权。我想用什么网卡是我说了算，不是联想说了算。

`PCI-E`是一个标准，就像`SATA`是硬盘的标准，`USB`是外设的一个标准，符合标准的硬件插上去后就应该能用，这是底线。

联想现在给网卡加白名单，难道以后，联想还要给硬盘加白名单，不是我指定的`SSD`不让你启动？给`USB`加白名单，非联想手机一律禁止充电？？！！

我认为联想设置白名单的行为构成不正当竞争，是对用户和其他硬件厂商权利的侵犯。国家应该出台政策干预这种行为。

此时此刻，我对联想电脑好感全无。

“理想和完整”？换来的是什么？难以理解的BIOS修改方法，变砖的风险，繁琐的过程，反复重启数十遍的测试，还不知道能不能成功。不说了，刷白名单去了！

数十分钟后更新：白名单成功刷入，原本被拒绝的wifi/蓝牙二合一无线网卡

工作的不能再正常。这充分说明联想的白名单完全是为了限制而限制，根本不是技术上的原因。因为从技术上来说，我这张被拒绝的卡是完全可以驱动的，并且我刷了白名单之后确实完美的驱动了！至于联想为什么要限制，无非是为了利益。用户体验？我换卡后的体验还不够好吗？联想无非是想让你买完电脑买他们家卡，多赚点钱罢了。

不知道有没有人在意过，为什么我们可以自由的组装电脑？我们可以从市场上买来一堆零件：机箱、电源、主板、CPU、内存、硬盘、显卡、显示器……然后简单的把插口插入插槽，再按下电源按钮——一台个人电脑

就诞生了。我们买来的这些零件，可能每个都是由不同的厂家生产的，这些厂家在不同的市场中甚至互为竞争对手。但是为什么这些零件组合在一起之后，就能准确无误的运行？它们天生就是这样的吗？

即使是选购笔记本，我们也有各种各样的选择。就算是同一个品牌，也会有AMD显卡和Nvidia显卡的笔记本同时出售。此外我们更加不会对给笔记本加一个非原装内存条或者换一个非原装硬盘这种事感到奇怪，其他厂商的内存条和硬盘插上去后能用是理所当然，如果不能用，那一定是内存条或者硬盘做的不好。

但是为什么一定要能用呢？为什么情况不是这样：只有特定厂家的配件才能在特定厂家的PC上工作？

究其原因，是因为现在我们用的个人电脑，无论是台式还是笔记本，全部都是当年IBM 5150 PC机的兼容机。

当年，IBM的Model 5150开创了通过独立的硬件提供商采购组件并进行组装的全新商业模式，从而造就了我们今天的PC硬件市场。Model 5150从一诞生就是开放的，IBM公开了其全部的技术手册
（无论是否基于自愿），因而任何厂商都可以生产Model 5150的兼容产品，任何厂商也都可以为Model 5150生产配件，并且这些配件可以在任何一台和Model 5150兼容的机器上正常工作。所以，今天我们才可以自由的选择我们的配件和我们的电脑。

而IBM PC之所以能够火起来，并且至今为止依然是个人电脑的行业标准，不就是因为其自由与开放吗？让每个厂商都能参与到其中，把选择权交给最终用户，并由市场决定下一个优秀标准是什么，而一但决定了标准，那么每个厂商都共同遵守。这是对所有人都有利的做法，这也是PC行业至今为止依然保持着活力的重要原因。

因此我觉得惠普和联想为笔记本加入无线模块白名单是没有什么意义的，它破坏了PC行业一贯的兼容性，导致了厂商专权和不正当竞争。

并且从事实上来说，它根本无法阻止真正想更换无线模块的用户——讽刺的是，惠普和联想用于阻止用户安装未授权无线模块的软件，那个被称为“BIOS”的系统固件，其本身的结构也是开放标准。因此用户可以用软件把BIOS的结构解开，剥离其中用于阻止未授权模块的代码，然后把其他部分完好无损的安装回去——就像他们对自己PC的硬件做的那样，把不好的零件替换掉。然后，一切限制也就没有了。

所以，封闭和控制不属于PC，开放和兼容才是PC的过去和现在，以及未来。并且，这一情况在另一“个人电脑”领域——手机行业也得到了淋漓尽致的体现。安卓正在越来越多的占领那些原本由封闭系统占领的市场份额。

参考阅读：

[PC发展史③ 个人PC开山鼻祖的前世今生_DIY攒机](https://link.zhihu.com/?target=http%3A//diy.zol.com.cn/541/5414339_all.html).

来自惠普社区的[一则回复](https://h30471.www3.hp.com/t5/bi-ji-ben-dian-nao/wang-ka-bai-ming-dan/td-p/826403).

> 白名单也好黑名单也好，是不会有官方答复承认或者否认存在的。笔记本设置所谓黑名单或白名单，用到的也是内部工具，不会对外公布。至于其他途径，有以前人为流出的内部工具，惠普也控制不了。

来自HPFOCUS的一个[工具分享](http://forum.hpfocus.com/viewtopic.php?f=2&t=3913).
